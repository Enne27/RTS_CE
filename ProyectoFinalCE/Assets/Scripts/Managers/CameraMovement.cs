...
public class CameraMovement : MonoBehaviour
{
    ...
    private void UpdateCameraPosition()
    {
        ...
        // 2. Calculamos la posici�n Z deseada usando ese porcentaje.
        // Si el porcentaje es 1 (lejos), Z ser� 0 (justo encima, visi�n vertical).
        // Si el porcentaje es 0 (cerca), Z ser� -maxZoomDistance (alejado, visi�n horizontal).
        float targetZ = Mathf.Lerp(-maxZoomDistance, 0f, zoomPercent);

        // 3. Creamos el target de posici�n local usando X=0 para mantenerla centrada.
        Vector3 zoomTarget = new Vector3(0f, zoomHeight, targetZ);

        // 4. Suavizamos la transici�n hacia ese punto local, usando tu variable zoomDampening.
        cameraTransform.localPosition = Vector3.Lerp(cameraTransform.localPosition, zoomTarget, Time.deltaTime * zoomDampening);

        // 5. Mirar al CameraRig. Al cambiar la posici�n en Z e Y, LookAt ajustar� el �ngulo X autom�ticamente[cite: 1].
        cameraTransform.LookAt(transform);
    }
